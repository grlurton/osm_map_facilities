...
DHISFacilities$Level5 <- gsub('PHC' , 'H/C' , DHISFacilities$Level5)
  ...
  if(nrow(lga_facilities@data) > 0){
    for(i in seq(nrow(lga_facilities@data))){
      ...
      if (!is.na(osm_name)){ 
        ...
        if (length(mm) == 0){
          dists <- stringdist(osm_name,facilities,method='jw')
          print(paste0('Looking for ' , osm_name))
          print(facilities[dists < 0.2])
        }
      }
